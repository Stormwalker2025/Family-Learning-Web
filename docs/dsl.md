...
const englishTFExample: TrueFalseQuestion = {
  ...
  answerKey: {
    correct: false,
    explanation: "Russia is the largest country in the world by land area. Australia is the 6th largest.",
...
const scienceMatchingExample: MatchingQuestion = {
  ...
  content: {
    instruction: "Drag each animal from the left to its correct habitat on the right",
    leftColumn: [
  ...
  answerKey: {
    ...
    explanation: "Each animal lives in a specific habitat that provides food and shelter."
  },
  metadata: {
    difficulty: "easy",
    yearLevel: 6,
...
const englishFillBlankExample: FillBlankQuestion = {
  ...
  answerKey: {
    answers: [
      {
        ...
        acceptSynonyms: true,
        points: 1,
        feedback: "Good! Animals that hop include kangaroos, rabbits, and frogs."
      },
      {
        blankId: "surface",
    ...
    explanation: "This sentence describes an animal moving across a surface to reach a destination."
  },
  metadata: {
    difficulty: "medium",
    yearLevel: 3,
...
class ErrorDiagnosisEngine {
  diagnoseMCQError(
    ...
    if (userAnswer.length === 0) {
      diagnoses.push({
        ...
        remediation: 'Encourage the student to read the question carefully and make their best guess',
        relatedConcepts: ['test-taking-strategies', 'reading-comprehension']
      });
    } else if (userAnswer.length > correctAnswer.length && correctAnswer.length === 1) {
      diagnoses.push({
        errorType: 'over_selection',
        confidence: 0.8,
        description: 'Student selected too many options for a single-answer question',
        remediation: 'Review instructions and look for keywords like "choose the best answer"',
    ...
    if (question.metadata.subject === 'mathematics') {
      diagnoses.push(...this.diagnoseMathErrors(question, userAnswer, correctAnswer));
    }
  ...
  private diagnoseMathErrors(
    ...
    if (question.metadata.tags.includes('division') && userAnswer.includes('a')) {
      ...
      if (divisionMatch) {
        ...
        if (question.content.options.some(opt => opt.text === multiplicationResult.toString() && userAnswer.includes(opt.id))) {
          diagnoses.push({
