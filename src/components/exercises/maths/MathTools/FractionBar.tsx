...
export const FractionBar: React.FC<FractionBarProps> = ({
  ...
  const [compareWith, setCompareWith] = useState<Fraction>({ numerator: 1, denominator: 3 })
  ...
  const findEquivalentFractions = (frac: Fraction): Fraction[] => {
    ...
    return equivalents.filter(eq => 
      !(eq.numerator === frac.numerator && eq.denominator === frac.denominator)
    ).slice(0, 4)
  }
  ...
  const renderFractionVisual = (frac: Fraction, color: string = 'blue', title?: string) => {
    const segments = []
    ...
    for (let i = 0; i < frac.denominator; i++) {
      ...
      segments.push(
        <rect
          ...
          height={60}
          fill={isShaded ? `${color === 'blue' ? '#3B82F6' : '#EF4444'}` : '#F3F4F6'}
          stroke="#6B7280"
          strokeWidth="1"
      ...
      <div className="space-y-6">
        ...
        {showingEquivalent && showEquivalent && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-center">Equivalent Fractions</h3>
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <div className="flex justify-center">
                      <svg width="150" height="30" className="border rounded">
                        {Array.from({ length: equiv.denominator }).map((_, i) => (
                          <rect
                            key={i}
        ...
        {/* Fraction Comparison */}
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-center">Compare Fractions</h3>
          
          <div className="flex items-center gap-4 justify-center">
            <Input
              type="number"
              value={compareWith.numerator}
              onChange={(e) => setCompareWith({
                ...compareWith,
                numerator: Math.max(0, parseInt(e.target.value) || 0)
              })}
              className="w-16"
          ...
          <div className="text-center p-4 bg-yellow-50 rounded-lg">
            ...
            <div className="text-sm text-gray-600 mt-2">
              {toDecimal(fraction).toFixed(3)} vs {toDecimal(compareWith).toFixed(3)}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="text-sm font-medium text-center">Quick Fractions:</div>
          <div className="flex flex-wrap justify-center gap-2">
            {[
              { num: 1, den: 2 }, { num: 1, den: 3 }, { num: 1, den: 4 },
              { num: 2, den: 3 }, { num: 3, den: 4 }, { num: 2, den: 5 },
              { num: 3, den: 5 }, { num: 5, den: 6 }
            ].map((frac, index) => (
              <Button
                key={index}
                variant="outline"
        ...
        <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded">
          ...
          <ul className="mt-1 space-y-1">
            <li>• Adjust numerator and denominator to create different fractions</li>
