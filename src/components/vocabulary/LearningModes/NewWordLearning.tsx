...
export function NewWordLearning({ onProgressUpdate }: NewWordLearningProps) {
  ...
  const startNewSession = useCallback(async () => {
    ...
    try {
      // 获取还未开始学习的词汇
      const response = await fetch('/api/vocabulary/progress?needsReview=false&phase=', {
        method: 'GET'
      });
      ...
      if (response.ok) {
        ...
        // 获取还未学习的词汇
        const unlearnedWords = data.progress?.filter((p: any) => 
          p.attempts === 0 || p.masteryLevel < 30
        ) || [];
        ...
        if (unlearnedWords.length === 0) {
          ...
          // 选择前5个未学习的词汇
          const wordsToLearn = unlearnedWords.slice(0, 5).map((p: any) => p.word);
  ...
  const handlePhaseComplete = async () => {
    ...
    const studyTime = Math.floor((new Date().getTime() - studyStartTime.getTime()) / 1000);
    ...
    // 切换到下一个阶段或下一个单词
    const currentPhaseIndex = LEARNING_PHASES.findIndex(p => p.id === currentPhase);
    ...
    if (currentPhaseIndex < LEARNING_PHASES.length - 1) {
      ...
      if (session.currentIndex < session.words.length - 1) {
        setSession(prev => prev ? {
          ...
          sessionStats: {
            ...prev.sessionStats,
            correctAnswers: isCorrect ? prev.sessionStats.correctAnswers + 1 : prev.sessionStats.correctAnswers
          }
        } : null);
        setCurrentPhase('introduction');
        setShowDefinition(false);
  ...
  const completeSession = () => {
    ...
    const accuracy = (session.sessionStats.correctAnswers / session.sessionStats.totalWords) * 100;
  ...
  // 生成练习选项
  const generateOptions = (correctAnswer: string, type: 'chinese' | 'english') => {
    if (!session) return [];
      ...
      <Card className="p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-4 text-sm text-gray-600">
            <span>单词 {session.currentIndex + 1} / {session.words.length}</span>
            <span>正确率 {Math.round((session.sessionStats.correctAnswers / Math.max(session.completedWords.length, 1)) * 100)}%</span>
          </div>
        </div>
        
        <Progress 
          value={((session.currentIndex * LEARNING_PHASES.length + LEARNING_PHASES.findIndex(p => p.id === currentPhase) + 1) / (session.words.length * LEARNING_PHASES.length)) * 100}
          className="h-3"
        />
      ...
      <Card className="p-4">
        <div className="flex justify-between items-center">
          {LEARNING_PHASES.map((phase, index) => {
            ...
            const isCompleted = LEARNING_PHASES.findIndex(p => p.id === currentPhase) > index;
        ...
        <div className="text-center space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-center space-x-3">
              <h1 className="text-4xl font-bold text-gray-900">{currentWord.word}</h1>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div className="flex justify-center space-x-2">
              ...
              <Badge className={`${
                currentWord.difficulty <= 2 ? 'bg-green-100 text-green-800' :
                currentWord.difficulty <= 3 ? 'bg-yellow-100 text-yellow-800' :
            ...
            <div className="space-y-6">
              <div className="bg-blue-50 p-6 rounded-lg">
                ...
                {currentWord.chineseDefinition && (
                  <p className="text-gray-700">{currentWord.chineseDefinition}</p>
                )}
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {currentWord.synonyms.length > 0 && (
                    <div className="bg-green-50 p-4 rounded-lg">
                      <h4 className="font-medium mb-2 text-green-800">同义词</h4>
                      <p className="text-green-700">{currentWord.synonyms.join(', ')}</p>
                    </div>
                  )}
                  {currentWord.antonyms.length > 0 && (
                    <div className="bg-red-50 p-4 rounded-lg">
                      <h4 className="font-medium mb-2 text-red-800">反义词</h4>
                      <p className="text-red-700">{currentWord.antonyms.join(', ')}</p>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="space-y-6">
              ...
              {!showResult && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {generateOptions(currentWord.chineseDefinition || currentWord.definition, 'chinese').map((option, index) => (
                    <Button
                      key={index}
                      variant="outline"
                      ...
                      onClick={() => {
                        ...
                        setIsCorrect(option === (currentWord.chineseDefinition || currentWord.definition));
                        setShowResult(true);
                      }}
                    >
                      {option}
                ...
                <div className={`p-6 rounded-lg ${isCorrect ? 'bg-green-50' : 'bg-red-50'}`}>
                  ...
                  {!isCorrect && (
                    <div className="mb-4">
                      ...
                      <p className="font-semibold">{currentWord.chineseDefinition || currentWord.definition}</p>
                    </div>
                  )}
          ...
          {currentPhase === 'comprehension' && (
            <div className="space-y-6">
              <div className="space-y-3">
                <h3 className="text-xl font-semibold">根据释义选择正确的单词</h3>
                <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="space-y-6">
              ...
              {showResult && (
                <div className={`p-6 rounded-lg ${isCorrect ? 'bg-green-50' : 'bg-red-50'}`}>
                  ...
                  <Button onClick={handlePhaseComplete}>
                    {session.currentIndex < session.words.length - 1 ? '下一个单词' : '完成学习'}
                    <ChevronRight className="h-4 w-4 ml-2" />
                  </Button>
                </div>
              )}
