...
export function TestMode({ onProgressUpdate }: TestModeProps) {
  ...
  // 生成测试题目
  const generateQuestions = (words: TestWord[], settings: any): TestQuestion[] => {
    ...
    words.forEach((word, index) => {
      const questionType = questionTypes[Math.floor(Math.random() * questionTypes.length)] as TestQuestion['type'];
  ...
  // 生成选择题选项
  const generateMultipleChoiceOptions = (correctAnswer: string, allAnswers: string[]): string[] => {
    const options = [correctAnswer];
    const availableAnswers = allAnswers.filter(answer => 
      answer !== correctAnswer && answer && answer.length > 0
    );
  ...
  const finishTest = () => {
    ...
    // 生成建议
    const recommendations = generateRecommendations(accuracy, categoriesScore, difficultyScore);
  ...
  const generateRecommendations = (
    ...
    if (accuracy >= 90) {
      recommendations.push('🎉 优秀！您的词汇掌握非常好，可以挑战更高难度的词汇。');
    } else if (accuracy >= 70) {
    ...
    // 难度建议
    const lowDifficultyAccuracy = difficultyScore[1] && difficultyScore[2] ?
      ((difficultyScore[1].correct + difficultyScore[2].correct) / 
       (difficultyScore[1].total + difficultyScore[2].total)) * 100 : 0;
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {TEST_SETTINGS.duration.map(option => (
                  <Button
                    key={option.value}
                    variant={testSettings.duration === option.value ? 'default' : 'outline'}
                    onClick={() => setTestSettings(prev => ({ ...prev, duration: option.value }))}
                  >
                    {option.label}
                  </Button>
                ))}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {TEST_SETTINGS.questionCount.map(option => (
                  <Button
                    key={option.value}
                    variant={testSettings.questionCount === option.value ? 'default' : 'outline'}
                    onClick={() => setTestSettings(prev => ({ ...prev, questionCount: option.value }))}
                  >
                    {option.label}
                  </Button>
                ))}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {TEST_SETTINGS.difficulty.map(option => (
                  <Button
                    key={option.value}
                    variant={testSettings.difficulty === option.value ? 'default' : 'outline'}
                    onClick={() => setTestSettings(prev => ({ ...prev, difficulty: option.value }))}
                  >
                    {option.label}
                  </Button>
                ))}
        ...
        <Card className="p-6 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            <div className="bg-blue-50 p-4 rounded-lg">
              <p className="text-2xl font-bold text-blue-600">{results.correctAnswers}</p>
            ...
            <div className="bg-orange-50 p-4 rounded-lg">
              <p className="text-2xl font-bold text-orange-600">{results.totalQuestions}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="p-6">
            ...
            <div className="space-y-3">
              {Object.entries(results.categoriesScore).map(([category, score]) => {
          ...
          <Card className="p-6">
            ...
            <div className="space-y-3">
              {Object.entries(results.difficultyScore).map(([difficulty, score]) => {
                const accuracy = (score.correct / score.total) * 100;
                return (
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-4">
              ...
              <Badge className={`${
                currentQuestion.word.difficulty <= 2 ? 'bg-green-100 text-green-800' :
                currentQuestion.word.difficulty <= 3 ? 'bg-yellow-100 text-yellow-800' :
        ...
        <Card className="p-8">
          <div className="text-center space-y-6">
            ...
            {(currentQuestion.type === 'fill_blank' || currentQuestion.type === 'spelling') && (
            ...
            {currentQuestion.userAnswer && (
              <div className={`p-6 rounded-lg ${
                ...
                {!currentQuestion.isCorrect && (
                  <div className="mb-3">
                    <p className="text-red-700">您的答案: {currentQuestion.userAnswer}</p>
                    <p className="text-red-700">正确答案: {currentQuestion.correctAnswer}</p>
                  </div>
                )}

                <div className="bg-white p-4 rounded border">
                  <p><strong>释义:</strong> {currentQuestion.word.definition}</p>
                  {currentQuestion.word.chineseDefinition && (
                    <p className="mt-1"><strong>中文:</strong> {currentQuestion.word.chineseDefinition}</p>
                  )}
                </div>
              </div>
            )}
