...
export function ReviewMode({ onProgressUpdate }: ReviewModeProps) {
  ...
  const [currentPracticeType, setCurrentPracticeType] =
    useState<string>('recognition')
  ...
  const startReviewSession = useCallback(async () => {
    ...
    try {
      ...
      const scheduleResponse = await fetch(
        '/api/vocabulary/review-schedule?includeOverdue=true'
      )
  ...
  const completeSession = () => {
    ...
    const accuracy =
      (session.sessionStats.correctAnswers /
        session.sessionStats.reviewedCount) *
      100
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex space-x-2">
            ...
            <Badge
              className={`${
                currentReviewWord.masteryLevel >= 80
                  ? 'bg-green-100 text-green-800'
                  : currentReviewWord.masteryLevel >= 50
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium">
                {new Date(
                  currentReviewWord.nextReviewDate
                ).toLocaleDateString()}
              </p>
            </div>
          </div>
        ...
        <div className="text-center space-y-6">
          ...
          {currentPracticeType === 'context' && (
            <div className="space-y-4">
              ...
              {!showResult && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {generateOptions(currentWord.word, 'context').map(
                    (option, index) => (
                      <Button
                        key={index}
                        variant="outline"
          ...
          {showResult && (
            ...
            >
              <div className="flex items-center justify-center space-x-2 mb-4">
                ...
                <span
                  className={`text-xl font-semibold ${isCorrect ? 'text-green-800' : 'text-red-800'}`}
                >
                  {isCorrect ? '正确！' : '错误'}
                </span>
              </div>
              ...
              <div className="bg-white p-4 rounded border mb-4">
                <p>
                  <strong>英文:</strong> {currentWord.definition}
                </p>
                {currentWord.chineseDefinition && (
                  <p className="mt-2">
                    <strong>中文:</strong> {currentWord.chineseDefinition}
                  </p>
                )}
                {currentWord.example && (
                  <p className="mt-2">
