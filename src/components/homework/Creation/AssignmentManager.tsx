...
export default function AssignmentManager({
  ...
  const filteredStudents = students.filter(student => {
    ...
    const matchesYearLevel = !filters.yearLevel || 
      student.yearLevel?.toString() === filters.yearLevel
    
    const matchesFamily = !filters.family || 
      student.familyId === filters.family
  ...
  const toggleSelectAll = () => {
    if (selectAll) {
      ...
      const newSelection = selectedStudents.filter(id => !visibleIds.includes(id))
      onUpdateSelection(newSelection)
    } else {
      // 选择当前可见的所有学生
      const visibleIds = filteredStudents.map(s => s.id)
  ...
  // 获取年级列表（用于过滤）
  const availableYearLevels = [...new Set(students.map(s => s.yearLevel).filter(Boolean))]
    .sort((a, b) => a! - b!)

  // 获取家庭列表（用于过滤）
  const availableFamilies = [...new Set(students.map(s => s.familyId).filter(Boolean))]
      ...
      {filteredStudents.length > 0 && (
        <Card className="p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {availableYearLevels.map(year => {
              const yearStudents = filteredStudents.filter(s => s.yearLevel === year)
              const allYearSelected = yearStudents.every(s => selectedStudents.includes(s.id))
              ...
              return (
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (allYearSelected) {
                      // 取消选择这个年级的所有学生
                      onUpdateSelection(selectedStudents.filter(id => !yearStudentIds.includes(id)))
                    } else {
                      // 选择这个年级的所有学生
                      onUpdateSelection([...new Set([...selectedStudents, ...yearStudentIds])])
                    }
                  }}
                >
                  {allYearSelected ? '取消' : '选择'} Year {year} ({yearStudents.length})
                </Button>
              )
            })}
          </div>
