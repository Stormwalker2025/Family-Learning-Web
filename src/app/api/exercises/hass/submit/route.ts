...
export async function POST(request: NextRequest) {
  try {
    ...
    const { score, feedback } = await evaluateHassSubmission(
      exercise,
      body.answers
    )
    ...
    const submission: HassSubmission = {
      ...
      rubricScores: feedback.questionAnalysis.reduce(
        (acc, analysis) => {
          if (analysis.rubricScores) {
            Object.entries(analysis.rubricScores).forEach(
              ([criterion, points]) => {
...
async function evaluateHassSubmission(
  ...
  for (const question of exercise.questions) {
    ...
    if (userAnswer && userAnswer.content) {
      ...
      switch (question.type) {
        ...
        case 'short-answer':
          ...
          questionScore = evaluateShortAnswer(
            question.correctAnswer,
            userAnswer.content,
            question.points
          )
    ...
    questionAnalyses.push({
      ...
      timeEfficiency:
        userAnswer.timeSpent < question.estimatedTime
          ? 'efficient'
          : 'adequate',
  ...
  const feedback: HassFeedback = {
    ...
    skillsAnalysis: Array.from(skillsAnalysis.entries()).map(
      ([skill, correct]) => ({
        skill,
        level:
          correct >= exercise.questions.length * 0.8
            ? 'proficient'
            : correct >= exercise.questions.length * 0.6
              ? 'developing'
              : correct >= exercise.questions.length * 0.4
                ? 'emerging'
                : ('needs-support' as any),
        evidence: [
          `Answered ${correct} out of ${exercise.questions.length} questions correctly`,
        ],
        nextSteps:
          correct < exercise.questions.length * 0.6
    ...
    conceptUnderstanding: exercise.learningObjectives.map(objective => ({
      ...
      misconceptions:
        totalScore < exercise.totalPoints * 0.6
          ? ['Review the key concepts']
          : undefined,
      supportNeeded:
        totalScore < exercise.totalPoints * 0.4
    ...
    curriculumAlignment: exercise.article.australianCurriculum.map(code => ({
      ...
      achievement:
        totalScore >= exercise.totalPoints * 0.8
          ? 'above'
          : totalScore >= exercise.totalPoints * 0.6
            ? 'at'
            : 'below',
      evidence: [
        `Exercise score: ${Math.round((totalScore / exercise.totalPoints) * 100)}%`,
      ],
    })),
  }
...
function evaluateExactMatch(correctAnswer: any, userAnswer: any): boolean {
  ...
  return correct.every(ans =>
    user.some(
      ua => String(ua).toLowerCase().trim() === String(ans).toLowerCase().trim()
    )
  )
}
...
function evaluateOpenEnded(question: any, userAnswer: HassAnswer): number {
  ...
  return Math.min(
    Math.round(complexity * 0.2 * question.points),
    question.points
  )
}

function assessThinkingDepth(
  userAnswer: HassAnswer,
  taxonomy: string
): 'surface' | 'developing' | 'deep' {
  const content = String(userAnswer.content)
...
function assessSourceAnalysis(questionAnalyses: any[]): number {
  const sourceQuestions = questionAnalyses.filter(
    qa => qa.questionType === 'source-analysis'
  )
...
function generateStrengths(
  ...
  const correctAnalysis = analyses.filter(
    qa => qa.isCorrect && ['analysis', 'evaluation'].includes(qa.questionType)
  ).length
...
function generateImprovements(
  ...
  const incorrectAnalysis = analyses.filter(
    qa => !qa.isCorrect && ['analysis', 'evaluation'].includes(qa.questionType)
  ).length
