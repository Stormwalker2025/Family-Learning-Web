...
const updateAssignmentSchema = z.object({
  ...
  status: z.enum(['ASSIGNED', 'IN_PROGRESS', 'COMPLETED', 'OVERDUE', 'REVIEWED']).optional(),
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const hasAccess = 
      ...
      (payload.role === 'PARENT' && await checkParentAccess(payload.userId, assignment.assignedTo.map(s => s.id)))
    ...
    const statistics = {
      ...
      submittedCount: assignment.submissions.filter(s => s.status === 'SUBMITTED' || s.status === 'COMPLETED').length,
      completedCount: assignment.submissions.filter(s => s.status === 'COMPLETED').length,
      overdueCount: assignment.submissions.filter(s => s.isLate).length,
      inProgressCount: assignment.submissions.filter(s => s.status === 'IN_PROGRESS').length,
      notStartedCount: assignment.submissions.filter(s => s.status === 'NOT_STARTED').length
    }

    // 计算平均分
    const scoresSubmissions = assignment.submissions.filter(s => s.totalScore !== null)
    const averageScore = scoresSubmissions.length > 0 
      ? scoresSubmissions.reduce((sum, s) => sum + (s.totalScore || 0), 0) / scoresSubmissions.length
      : null
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        statistics: {
          ...statistics,
          averageScore,
          completionRate: statistics.totalStudents > 0 ? (statistics.completedCount / statistics.totalStudents) * 100 : 0
        }
      }
    })
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // 权限检查：只有创建者或管理员可以修改
    if (payload.role !== 'ADMIN' && existingAssignment.assignedBy !== payload.userId) {
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const hasActiveSubmissions = existingAssignment.submissions.some(
      s => s.status === 'IN_PROGRESS' || s.status === 'COMPLETED' || s.status === 'SUBMITTED'
    )
