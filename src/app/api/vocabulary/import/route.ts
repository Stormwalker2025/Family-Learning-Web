    ...
    try {
      ...
      const mappedRecords = records.map((record: any, index: number) => {
        ...
        Object.keys(record).forEach(key => {
          ...
          const mappedKey =
            CSV_FIELD_MAPPING[
              normalizedKey as keyof typeof CSV_FIELD_MAPPING
            ] || normalizedKey
          if (record[key]) {
        ...
        try {
          const wordData = {
            ...
            synonyms: record.synonyms
              ? JSON.stringify(record.synonyms.split(',').map(s => s.trim()))
              : null,
            antonyms: record.antonyms
              ? JSON.stringify(record.antonyms.split(',').map(s => s.trim()))
              : null,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const processedImports = imports.map(imp => ({
      ...
      importSettings: imp.importSettings
        ? JSON.parse(imp.importSettings)
        : null,
    }))
