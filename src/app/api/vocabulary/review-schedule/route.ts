...
export async function GET(request: NextRequest) {
  try {
    ...
    const statistics = {
      ...
      overdueWords: reviewWords.filter(
        p => p.nextReviewDate && p.nextReviewDate < today
      ).length,
      ...
      upcomingWords: reviewWords.filter(
        p => p.nextReviewDate && p.nextReviewDate > today
      ).length,
      ...
      masteryDistribution: {
        ...
        medium: reviewWords.filter(
          p => p.masteryLevel >= 30 && p.masteryLevel < 70
        ).length,
...
function generateRecommendations(
  ...
  if (statistics.overdueWords > 10) {
    recommendations.push(
      `您有 ${statistics.overdueWords} 个词汇已过期复习，建议优先复习这些词汇`
    )
  }
  ...
  if (lowMasteryWords > 5) {
    recommendations.push(
      `有 ${lowMasteryWords} 个词汇掌握度较低，建议增加练习频率`
    )
  }
