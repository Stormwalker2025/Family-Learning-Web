...
const ProgressUpdateSchema = z.object({
  ...
  phase: z
    .enum(['RECOGNITION', 'UNDERSTANDING', 'APPLICATION', 'MASTERY'])
    .optional(),
  isCorrect: z.boolean(),
  timeSpent: z.number().min(0).optional(), // 秒
  practiceType: z
    .enum(['recognition', 'translation', 'spelling', 'listening', 'context'])
    .optional(),
})
...
function calculateNextReview(
  currentLevel: number,
  isCorrect: boolean
): { nextDate: Date; interval: number; newLevel: number } {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newCorrectAttempts = validatedData.isCorrect
      ? progress.correctAttempts + 1
      : progress.correctAttempts
    const newStreakCount = validatedData.isCorrect
      ? progress.streakCount + 1
      : 0
    const newTotalStudyTime =
      progress.totalStudyTime + (validatedData.timeSpent || 0)

    // 计算下次复习时间
    const { nextDate, interval, newLevel } = calculateNextReview(
      progress.ebbinghausLevel,
    ...
    const updatedProgress = await prisma.vocabularyProgress.update({
      ...
      data: {
        ...
        lastSeen: new Date(),
        lastCorrect: validatedData.isCorrect
          ? new Date()
  ...
  try {
    ...
    const [progressList, total] = await Promise.all([
      prisma.vocabularyProgress.findMany({
        ...
        skip,
        take: limit,
        orderBy: [{ nextReviewDate: 'asc' }, { lastSeen: 'desc' }],
        include: {
          word: {
            select: {
              id: true,
    ...
    const todayReviews = reviewStats.filter(
      r => r.nextReviewDate && r.nextReviewDate <= new Date()
    ).length

    const tomorrowReviews = reviewStats.filter(
      r =>
        r.nextReviewDate &&
