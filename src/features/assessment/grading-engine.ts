...
export class GradingEngine {
    ...
    if (timeTaken && question.scoring.bonusForSpeed) {
      ...
      const adjustedScore = Math.max(
        ...
        Math.min(
          100,
          result.score +
            timeAdjustment.bonus * 100 -
            timeAdjustment.penalty * 100
        )
      )
   ...
   */
  private async gradeMCQ(
    question: MCQQuestion,
    userAnswer: string[]
  ): Promise<Omit<GradingResult, 'gradingMetadata'>> {
    ...
    const incorrectSelected = [...userAnswers].filter(
      ans => !correctAnswers.has(ans)
    )
    const correctMissed = [...correctAnswers].filter(
      ans => !userAnswers.has(ans)
    )
    ...
    if (question.scoring.partialCredit && correctAnswers.size > 1) {
      ...
      isCorrect =
        correctSelected.length === correctAnswers.size &&
    ...
    const feedback = this.generateMCQFeedback(
      question,
      userAnswers,
      correctAnswers
    )
  ...
  private async gradeTrueFalse(
    ...
    if (isCorrect) {
      ...
      feedback.push(
        `❌ Incorrect. The correct answer is ${question.answerKey.correct ? 'True' : 'False'}.`
      )
    }
  ...
  private async gradeMatching(
    ...
    userAnswer: MatchPair[]
  ): Promise<Omit<GradingResult, 'gradingMetadata'>> {
    const correctMatches = new Map(
      question.answerKey.matches.map(match => [
    ...
    if (isOverallCorrect) {
      ...
      feedback.push(
        `You got ${correctCount} out of ${totalExpected} matches correct.`
      )
    }
  ...
  private async gradeFillBlank(
    ...
    for (const blank of question.content.blanks) {
      const answerDef = question.answerKey.answers.find(
        a => a.blankId === blank.id
      )
      ...
      const gradeResult = this.gradeBlankAnswer(
        userInput,
        answerDef,
        question.content.caseSensitive
      )
  ...
  private generateMCQFeedback(
    ...
    if (isFullyCorrect) {
      ...
      if (correctSelected.length > 0) {
        const correctOptions = correctSelected
          .map(
            id =>
      ...
      if (incorrectSelected.length > 0) {
        const incorrectOptions = incorrectSelected
          .map(
            id =>
              question.content.options.find(opt => opt.id === id)?.text || id
          )
  ...
  private generateFillBlankFeedback(
    ...
    if (correctCount === totalCount) {
      ...
      feedback.push(
        `You got ${correctCount} out of ${totalCount} blanks correct.`
      )
    } else {
      feedback.push(
        '❌ None of the blanks were filled correctly. Please review your answers.'
      )
    }
  ...
  private calculateTimeAdjustment(
    ...
    if (timeTaken <= config.speedBonusThreshold) {
      ...
      message =
        '⏰ This took longer than expected. Try to work more quickly next time.'
    }
  ...
  private diagnoseMCQErrors(
    ...
    if (!userAnswer || userAnswer.length === 0) {
      diagnoses.push({
        ...
        remediation:
          'Make sure to read the question carefully and select the best answer',
      ...
      diagnoses.push({
        ...
        remediation:
          'Look for keywords like "choose the best answer" or "select one"',
  ...
  private diagnoseMathErrors(
    ...
    if (question.metadata.tags.includes('division')) {
      ...
      if (divisionMatch) {
        ...
        const selectedMultiplication = question.content.options.find(
          opt =>
  ...
  private diagnoseFillBlankErrors(
    question: FillBlankQuestion,
    userAnswer: Record<string, string>
  ): ErrorDiagnosis[] {
    const diagnoses: ErrorDiagnosis[] = []

    const emptyBlanks = question.content.blanks.filter(
      blank => !userAnswer[blank.id]?.trim()
    ).length
  ...
  private generateSuggestions(
    ...
    if (result.score < 50) {
      suggestions.push(
        `Review the ${question.metadata.subject} topic: ${question.metadata.topic}`
      )
      suggestions.push(
        'Consider asking your teacher for additional help with this concept'
      )
    } else if (result.score < 80) {
      suggestions.push(
        'Good effort! A little more practice will help you master this topic'
      )
      suggestions.push('Try similar questions to reinforce your understanding')
    } else {
      suggestions.push(
        'Great work! You have a solid understanding of this concept'
      )
      if (result.score < 100) {
        suggestions.push(
          'Pay attention to small details to achieve perfect scores'
        )
      }
    }
