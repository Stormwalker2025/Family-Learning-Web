...
export class RuleEvaluator {
   ...
   */
  async evaluateRules(request: UnlockEvaluationRequest, rules: UnlockRule[]): Promise<UnlockEvaluationResponse> {
    ...
    const finalRules = await this.applyLimitsAndCooldowns(triggeredRules, request.userId, sortedRules);
    
    // Combine results
    const response = this.combineResults(finalRules, request.userId, Date.now() - startTime, evaluationResults.length);
   ...
   */
  private async evaluateRule(rule: UnlockRule, context: EvaluationContext): Promise<RuleEvaluationResult> {
    const startTime = Date.now();
    ...
    try {
      // Check each criteria
      const checks = [
        () => this.checkSubjectCriteria(rule.criteria, context, criteriaMatched, criteriaFailed),
        () => this.checkYearLevelCriteria(rule.criteria, context, criteriaMatched, criteriaFailed),
        () => this.checkScoreCriteria(rule.criteria, context, criteriaMatched, criteriaFailed),
        () => this.checkTimeCriteria(rule.criteria, context, criteriaMatched, criteriaFailed),
        () => this.checkExerciseTypeCriteria(rule.criteria, context, criteriaMatched, criteriaFailed),
        () => this.checkDifficultyCriteria(rule.criteria, context, criteriaMatched, criteriaFailed),
        () => this.checkTimeOfDayCriteria(rule.criteria, context, criteriaMatched, criteriaFailed),
        () => this.checkDayOfWeekCriteria(rule.criteria, context, criteriaMatched, criteriaFailed),
        () => this.checkBehaviorModifiers(rule.criteria, context, criteriaMatched, criteriaFailed),
        () => this.checkCustomConditions(rule.criteria, context, criteriaMatched, criteriaFailed)
      ];
  ...
  private checkSubjectCriteria(
    ...
    const subjectMatches = criteria.subject.includes(context.subject) || criteria.subject.includes('all');
    
    if (subjectMatches) {
      matched.push(`subject:${context.subject}`);
    } else {
      failed.push(`subject:${context.subject}_not_in_${criteria.subject.join(',')}`);
    }
  ...
  private checkYearLevelCriteria(
    ...
    if (yearLevelMatches) {
      ...
      failed.push(`yearLevel:${context.yearLevel}_not_in_${criteria.yearLevel.join(',')}`);
    }
  ...
  private checkTimeCriteria(
    ...
    if (criteria.timeLimit.maxSeconds !== undefined) {
      ...
      if (maxTimeCheck) {
        matched.push(`maxTime:${context.timeTaken}<=${criteria.timeLimit.maxSeconds}`);
      } else {
        failed.push(`maxTime:${context.timeTaken}>${criteria.timeLimit.maxSeconds}`);
      }
    }
    ...
    if (criteria.timeLimit.bonusUnderSeconds !== undefined) {
      ...
      if (bonusTimeCheck) {
        matched.push(`bonusTime:${context.timeTaken}<=${criteria.timeLimit.bonusUnderSeconds}`);
      } else {
        // Note: This doesn't fail the rule, it just doesn't get the bonus
        matched.push(`bonusTime:not_eligible:${context.timeTaken}>${criteria.timeLimit.bonusUnderSeconds}`);
      }
    }
  ...
  private checkExerciseTypeCriteria(
    ...
    if (!criteria.exerciseType || criteria.exerciseType.length === 0) return true;
    
    const exerciseTypeMatches = criteria.exerciseType.includes(context.exerciseType);
    
    if (exerciseTypeMatches) {
      matched.push(`exerciseType:${context.exerciseType}`);
    } else {
      failed.push(`exerciseType:${context.exerciseType}_not_in_${criteria.exerciseType.join(',')}`);
    }
  ...
  private checkDifficultyCriteria(
    ...
    if (difficultyMatches) {
      ...
      failed.push(`difficulty:${context.difficulty}_not_in_${criteria.difficulty.join(',')}`);
    }
  ...
  private checkTimeOfDayCriteria(
    ...
    const timeMatches = currentHour >= criteria.timeOfDay.startHour && currentHour <= criteria.timeOfDay.endHour;
    
    if (timeMatches) {
      matched.push(`timeOfDay:${currentHour}_in_${criteria.timeOfDay.startHour}-${criteria.timeOfDay.endHour}`);
    } else {
      failed.push(`timeOfDay:${currentHour}_not_in_${criteria.timeOfDay.startHour}-${criteria.timeOfDay.endHour}`);
    }
  ...
  private checkDayOfWeekCriteria(
    ...
    if (dayMatches) {
      ...
      failed.push(`dayOfWeek:${currentDay}_not_in_${criteria.dayOfWeek.join(',')}`);
    }
  ...
  private checkBehaviorModifiers(
    ...
    if (criteria.behaviorModifiers.completionStreak !== undefined) {
      const streakMatches = (context.currentStreak || 0) >= criteria.behaviorModifiers.completionStreak;
      if (streakMatches) {
        matched.push(`completionStreak:${context.currentStreak}>=${criteria.behaviorModifiers.completionStreak}`);
      } else {
        failed.push(`completionStreak:${context.currentStreak || 0}<${criteria.behaviorModifiers.completionStreak}`);
    ...
    if (criteria.behaviorModifiers.improvementRate !== undefined) {
      const improvement = this.calculateImprovementRate(context.recentPerformance || []);
      const improvementMatches = improvement >= criteria.behaviorModifiers.improvementRate;
      if (improvementMatches) {
        matched.push(`improvementRate:${improvement.toFixed(1)}>=${criteria.behaviorModifiers.improvementRate}`);
      } else {
        failed.push(`improvementRate:${improvement.toFixed(1)}<${criteria.behaviorModifiers.improvementRate}`);
        allBehaviorChecks = false;
      }
    }
    ...
    if (criteria.behaviorModifiers.mistakeReduction !== undefined) {
      const mistakeReduction = this.calculateMistakeReduction(context.recentPerformance || []);
      const reductionMatches = mistakeReduction >= criteria.behaviorModifiers.mistakeReduction;
      if (reductionMatches) {
        matched.push(`mistakeReduction:${mistakeReduction.toFixed(2)}>=${criteria.behaviorModifiers.mistakeReduction}`);
      } else {
        failed.push(`mistakeReduction:${mistakeReduction.toFixed(2)}<${criteria.behaviorModifiers.mistakeReduction}`);
        allBehaviorChecks = false;
      }
    }
  ...
  private checkCustomConditions(
    ...
    if (!criteria.customConditions || criteria.customConditions.length === 0) return true;
    
    for (const condition of criteria.customConditions) {
      const conditionMet = this.evaluateCustomCondition(condition, context);
      if (conditionMet) {
        matched.push(`custom:${condition.field}_${condition.operator}_${condition.value}`);
      } else {
        failed.push(`custom:${condition.field}_${condition.operator}_${condition.value}`);
  ...
  private calculateImprovementRate(recentPerformance: any[]): number {
    ...
    const sorted = recentPerformance.sort((a, b) => new Date(a.completedAt).getTime() - new Date(b.completedAt).getTime());
  ...
  private calculateMistakeReduction(recentPerformance: any[]): number {
    ...
    const firstHalfMistakeRate = 1 - (firstHalf.reduce((sum, p) => sum + p.score, 0) / firstHalf.length / 100);
    const secondHalfMistakeRate = 1 - (secondHalf.reduce((sum, p) => sum + p.score, 0) / secondHalf.length / 100);
    
    return Math.max(0, firstHalfMistakeRate - secondHalfMistakeRate);
  }

  private evaluateCustomCondition(condition: CustomCondition, context: any): boolean {
  ...
  private calculateConfidence(matchedCriteria: string[], criteria: RuleCriteria): number {
   ...
   */
  private filterApplicableRules(rules: UnlockRule[], request: UnlockEvaluationRequest): UnlockRule[] {
    const now = new Date();
  ...
  private combineResults(
    ...
    for (const result of triggeredRules) {
      ...
      if (result.achievements) {
        allAchievements.push(...result.achievements.filter(a => !allAchievements.includes(a)));
      }
      ...
      if (result.restrictions) {
        if (result.restrictions.allowedApps) {
          combinedRestrictions.allowedApps.push(...result.restrictions.allowedApps);
        }
        if (result.restrictions.blockedApps) {
          combinedRestrictions.blockedApps.push(...result.restrictions.blockedApps);
        }
        if (result.restrictions.timeWindows) {
          combinedRestrictions.timeWindows.push(...result.restrictions.timeWindows);
        }
      }
    }
    
    // Remove duplicates from restrictions
    combinedRestrictions.allowedApps = [...new Set(combinedRestrictions.allowedApps)];
    combinedRestrictions.blockedApps = [...new Set(combinedRestrictions.blockedApps)];
    ...
    const summary: EvaluationSummary = {
      ...
      highestPriorityRule: triggeredRules.length > 0 ? triggeredRules[0].ruleId : undefined,
   ...
   */
  validateRule(rule: Partial<UnlockRule>): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
    ...
    if (rule.criteria) {
      ...
      if (rule.criteria.minScore !== undefined && (rule.criteria.minScore < 0 || rule.criteria.minScore > 100)) {
        errors.push('Minimum score must be between 0 and 100');
      }
      
      if (rule.criteria.maxScore !== undefined && (rule.criteria.maxScore < 0 || rule.criteria.maxScore > 100)) {
        errors.push('Maximum score must be between 0 and 100');
      }
      
      if (rule.criteria.minScore !== undefined && rule.criteria.maxScore !== undefined && 
    ...
    // Validate action
    if (rule.action) {
      if (rule.action.unlockMinutes === undefined || rule.action.unlockMinutes < 0) {
    ...
    if (rule.limits) {
      ...
      if (rule.limits.maxDaily !== undefined && rule.limits.maxWeekly !== undefined &&
