    ...
    try {
      ...
      if (config.rateLimit) {
        const rateLimitResult = await rateLimit({
          ...
          skipSuccessful: config.rateLimit.skipSuccessfulRequests
        });
      ...
      if (config.inputValidation?.enabled && ['POST', 'PUT', 'PATCH'].includes(method)) {
        try {
          const requestData = await getRequestBody(request);
          const validationResult = validateInput(requestData, config.inputValidation.rules || []);
      ...
      if (config.authentication?.required) {
        ...
        if (config.authentication.allowedRoles && 
            !config.authentication.allowedRoles.includes(authResult.user?.role || '')) {
          await auditLogger.logSecurityEvent({
 ...
 */
function validateCors(request: NextRequest, corsConfig: NonNullable<SecurityConfig['cors']>) {
...
function detectSuspiciousInput(data: any): string[] {
  ...
  const sqlPatterns = [
    'union select', 'drop table', 'drop database', 'xp_cmdshell',
    'sp_executesql', '--', ';--', 'exec(', 'execute(',
    'union all select', 'insert into', 'delete from', 'update set'
  ];
  
  // XSS patterns
  const xssPatterns = [
    '<script', 'javascript:', 'onerror=', 'onload=', 'onclick=',
    'vbscript:', 'data:text/html', 'eval(', 'alert(', 'document.cookie'
  ];
  
  // Path traversal patterns
  const pathTraversalPatterns = [
    '../', '..\\', '..\/', '%2e%2e%2f', '%2e%2e%5c', 
    '/etc/passwd', '/etc/shadow', 'c:\\windows', '/proc/version'
  ];
  
  // Command injection patterns
  const commandInjectionPatterns = [
    '|', '&', ';', '$(', '`', 'wget ', 'curl ', 'nc ', 'netcat',
    'chmod ', 'chown ', 'rm -rf', 'find /', 'cat /etc'
  ];
  ...
  const allPatterns = [
    ...
    ...commandInjectionPatterns.map(p => ({ pattern: p, type: 'COMMAND_INJECTION' }))
  ];
...
export const publicApiSecurity = withSecurity({
  rateLimit: { requests: 50, window: 900 }, // 50 requests per 15 minutes
  inputValidation: { enabled: true },
...
export const adminApiSecurity = withSecurity({
  rateLimit: { requests: 200, window: 900 }, // 200 requests per 15 minutes
